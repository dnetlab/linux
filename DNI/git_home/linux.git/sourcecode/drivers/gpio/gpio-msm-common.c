...
static struct proc_dir_entry *cloud_led = NULL;
static struct proc_dir_entry *vpn_led = NULL;
int led_factory = 0;
int button_factory = 0;
int cloud_led_status = R7500_LED_OFF;
int vpn_led_status = R7500_LED_OFF;
...
static int cloud_led_read(char *page, char **start, off_t off,
                               int count, int *eof, void *data)
{
    return sprintf(page, "%d\n", cloud_led_status);
}

static int cloud_led_write(struct file *file, const char *buf,
                                        unsigned long count, void *data)
{
    u_int32_t val;
    if (sscanf(buf, "%d", &val) != 1)
            return -EINVAL;
    if (!val) {
        cloud_led_status = R7500_LED_OFF;
        msm_gpio_set(r7500_gpio_chip, R7500_GPIO_LED_USB3, R7500_LED_OFF);
    } else {    
        cloud_led_status = R7500_LED_ON;
        msm_gpio_set(r7500_gpio_chip, R7500_GPIO_LED_USB3, R7500_LED_ON);
    }
    return count;
}

static int vpn_led_read(char *page, char **start, off_t off,
    int count, int *eof, void *data)
{
    return sprintf(page, "%d\n", vpn_led_status);
}

static int vpn_led_write(struct file *file, const char *buf,
    unsigned long count, void *data)
{
    u_int32_t val;
    if (sscanf(buf, "%d", &val) != 1)
            return -EINVAL;
    if (!val) {
        vpn_led_status = R7500_LED_OFF;
        msm_gpio_set(r7500_gpio_chip, R7500_GPIO_LED_SATA, R7500_LED_OFF);
    } else {    
        vpn_led_status = R7500_LED_ON;
        msm_gpio_set(r7500_gpio_chip, R7500_GPIO_LED_SATA, R7500_LED_ON);
    }
    return count;
}
...
static int create_simple_config_led_proc_entry(void)
{
    ...
    cloud_led = create_proc_entry("cloud_led", 0644, simple_config_entry);
	if (!cloud_led)
			return -ENOENT;
	cloud_led->write_proc = cloud_led_write;
	cloud_led->read_proc = cloud_led_read;

    vpn_led = create_proc_entry("vpn_led", 0644, simple_config_entry);
	if (!vpn_led)
			return -ENOENT;
	vpn_led->write_proc = vpn_led_write;
	vpn_led->read_proc = vpn_led_read;
